...
def add_word(story_id, user_id, word):
    ...
    """
    if not word or ' ' in word or len(word) > 45:
        return InvalidWord
...
def to_dicts(row_proxy):
    return [dict(r.items()) for r in row_proxy]

def recently_completed_stories():
    return to_dicts(db.session.execute('''
        SELECT s.id AS story_id
            , JSON_AGG(f.word ORDER BY f.created_at)  AS words
